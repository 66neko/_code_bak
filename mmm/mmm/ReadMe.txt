...
应用程序向导已为您创建了此 mmm 项目，作为编写可执行程序 (EXE) 的起点。
...
mmm.idl
    ...
    此文件将由 MIDL 编译器进行处理以生成：
        C++ 接口定义和 GUID 声明 (mmm.h)
        GUID 定义                                (mmm_i.c)
        类型库                                  (mmm.tlb)
        封送处理代码                                 （mmm_p.c 和 dlldata.c）

mmm.h
    此文件包含 mmm.idl 中定义的项目的 C++ 接口定义和 GUID 声明。它将在编译过程中由 MIDL 重新生成。
...
mmmps.vcproj
	...
	dlldata.c、mmm_i.c 和 mmm_p.c，这些文件是
